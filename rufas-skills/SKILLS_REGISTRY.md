...
| `prework-educacional-account-intelligence` | ACTIVE | Construir inteligência de conta de instituição de ensino antes de abordagem comercial NAR. | pesquisa/inteligência de conta | YES | NO | NO | n8n/Data Tables; CRM é downstream/exportação posterior | Version 1.1.0 validada no runtime; `PREPARE_ONLY`; stakeholder mapping + deep enrichment PRIMARY/SECONDARY; sem outbound automático. |
...
| `educational-opportunity-discovery-brazil` | DRAFT | Descobrir e qualificar sinais comerciais em instituições privadas de ensino no Brasil. | discovery / qualification | YES | NO | NO | n8n/Data Tables | Produz candidatos; Pré-Work profundo acontece em `prework-educacional-account-intelligence`. Não controla cron/agenda/limite diário/retry/fila/watermark/dedupe operacional/Google Sheets/CRM — isso é n8n. |
