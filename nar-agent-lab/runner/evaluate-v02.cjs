#!/usr/bin/env node
/**
 * Avalia os outputs do RUN_01 do benchmark v0.2 contra o gabarito v0.2.
 *
 * Lê EXCLUSIVAMENTE:
 *   - benchmarks/v0.2/router-cases-v0.2.intents.json (só para contagem/sanidade)
 *   - benchmarks/v0.2/router-cases-v0.2.expected.json (gabarito)
 *   - capabilities/capability-registry.json (tool policy)
 *   - runner/outputs/v02-run-01/*.json (outputs a avaliar)
 *
 ...
 *
 * Não modifica nenhum output cru — apenas lê e agrega em runner/results-v02/.
 *
...
const FORBIDDEN_V01_PATHS = [
  'benchmarks/router-cases.json',
  'runner/evaluate.cjs',
  'runner/results/',
  'runner/outputs/run-01',
  'runner/outputs/run-02',
  'runner/outputs/run-03',
];
// Guarda dura: este script não pode ler nenhum artefato do v0.1.
for (const p of FORBIDDEN_V01_PATHS) {
...
const outDir = dirArg > -1
  ...
  : path.join(__dirname, 'outputs', `v02-run-${RUN}`);

const expectedFile = J('benchmarks/v0.2/router-cases-v0.2.expected.json');
const intentsFile = J('benchmarks/v0.2/router-cases-v0.2.intents.json');
...
if (cases.length !== 34) { console.error(`ESPERADO 34 casos no gabarito, achei ${cases.length}`); process.exit(1); }
...
if (missing.length) {
  console.error(`OUTPUTS AUSENTES OU INVÁLIDOS (${missing.length}/34): ${missing.join(', ')}`);
...
const report = {
  run: `v02-${RUN}`, suite: 'router-cases-v0.2', model: 'sonnet',
...
fs.mkdirSync(path.join(__dirname, 'results-v02'), { recursive: true });
fs.writeFileSync(path.join(__dirname, 'results-v02', `run-${RUN}.json`), JSON.stringify(report, null, 2) + '\n');
