...
## Tools por agente (capability registry 0.2.0)
...
#### `eduinfo_list_root`
- **entidade:** drive_asset · **temporalidade:** static_document
- **observa:**
  - itens autorizados na raiz do acervo: id, nome, mimeType, se e pasta, tamanho
- **não observa:**
  - conteudo interno do arquivo
  - classificacao semantica do documento (que assunto o arquivo trata)
  - telemetria, logs, latencia ou uptime de sistema
- **observável ao exercitar a tool:**
  - disponibilidade e autorizacao do acervo (ACCESS_DENIED, ASSET_OUT_OF_SCOPE)
  - existencia do item (FOLDER_NOT_FOUND, FILE_NOT_FOUND)

#### `eduinfo_search_folder`
- **entidade:** drive_asset · **temporalidade:** static_document
- **observa:**
  - arquivos e pastas dentro de um Folder_ID autorizado: id, nome, mimeType, se e pasta, tamanho
- **não observa:**
  - conteudo interno do arquivo
  - classificacao semantica do documento (que assunto o arquivo trata)
  - telemetria, logs, latencia ou uptime de sistema
- **observável ao exercitar a tool:**
  - disponibilidade e autorizacao do acervo (ACCESS_DENIED, ASSET_OUT_OF_SCOPE)
  - existencia da pasta (FOLDER_NOT_FOUND)

#### `eduinfo_get_asset`
- **entidade:** drive_asset · **temporalidade:** static_document
- **observa:**
  - um asset autorizado de forma consumivel: nome, mimeType, disponibilidade, URL temporaria
- **não observa:**
  - classificacao semantica do documento (que assunto o arquivo trata)
  - telemetria, logs, latencia ou uptime de sistema
- **observável ao exercitar a tool:**
  - disponibilidade e autorizacao do asset (ACCESS_DENIED, ASSET_OUT_OF_SCOPE, FILE_NOT_FOUND)

#### `crm_list_eligible_contacts`
- **entidade:** contact · **temporalidade:** current_state
- **observa:**
  - contatos aptos a cadencia numa data de referencia: id, nome, email, stage ATUAL, segmento, escola, ultima interacao
- **não observa:**
  - transicao de stage ao longo do tempo (o campo stage e um valor unico do estado atual, nao uma serie)
  - motivo da mudanca de fase

#### `crm_get_contact_context`
- **entidade:** contact · **temporalidade:** current_state + event_history
- **observa:**
  - stage ATUAL do contato (valor unico)
  - historico de interacao datado: date, channel, summary
  - ultima interacao, status de demo, status de follow-up
  - dados do contato: id, nome, email, telefone, escola, segmento
- **não observa:**
  - transicao de stage ao longo do tempo: o historico registra INTERACOES (data/canal/resumo), nao mudancas de fase; nao ha como derivar quantos contatos mudaram de stage num periodo
  - motivo da mudanca de fase

#### `crm_get_followups_due`
- **entidade:** followup · **temporalidade:** current_state
- **observa:**
  - follow-ups vencidos ou devidos ate uma data: contato, stage atual, data devida, status, dias em atraso
- **não observa:**
  - transicao de stage ao longo do tempo
  - conteudo da comunicacao de follow-up

#### `crm_get_demo_scheduled`
- **entidade:** demo · **temporalidade:** current_state
- **observa:**
  - demos agendadas num intervalo de datas: contato, escola, data/hora agendada, stage atual
- **não observa:**
  - transicao de stage ao longo do tempo
  - resultado ou conteudo da demo realizada

#### `proposal_get_pending`
- **entidade:** proposal · **temporalidade:** current_state
- **observa:**
  - propostas aguardando acao: id, contato, escola, titulo, status, valor, data de envio, validade, dias esperando
- **não observa:**
  - historico de versoes da proposta
  - politica comercial que originou a proposta

#### `campaign_get_history`
- **entidade:** campaign · **temporalidade:** event_history
- **observa:**
  - campanhas e templates anteriores: marca, nome, canal, status, datas de inicio/fim, template, performance
- **não observa:**
  - campanhas de marca nao registrada (BRAND_UNKNOWN)
  - telemetria de entrega em tempo real

#### `campaign_get_brand_context`
- **entidade:** brand · **temporalidade:** static_document
- **observa:**
  - posicionamento vigente da marca
  - guardrails de marca
  - mensagens aprovadas
  - claims restritos
- **não observa:**
  - politicas operacionais fora do escopo de marca (carencia, inadimplencia, prazos contratuais)
  - marca nao registrada (BRAND_UNKNOWN)

### Não observado por nenhuma tool

Declaração de mundo fechado: nada no contrato nar-ops-mcp@0.2.0 observa os itens
abaixo. Uma intenção que dependa de um deles é capability gap genuíno.

  - transicao de stage ao longo do tempo (quantos contatos mudaram de fase num periodo) — nenhuma tool expoe serie historica de stage
  - telemetria, logs, latencia, uptime ou metricas de sistema em producao
  - historico de deploy, versoes publicadas ou rollback aplicado
  - algoritmo de scoring de leads, sua configuracao ou seu desempenho
  - folha de pagamento, dados financeiros de RH
  - repositorio de contratos juridicos
  - qualquer capability de ESCRITA (enviar, publicar, compartilhar, alterar CRM, criar proposta) — ver not_yet_created
...
capability disponível:
  ...
  - Relatos de bug / issue tracker fazem parte do acervo observavel pelas tools eduinfo_*?
    As tools de acervo devolvem itens por nome/mimeType e NAO classificam o assunto do documento. O contrato nao afirma nem nega que relatos de bug existam como documentos no acervo autorizado.
...
> A missão anterior travou por um erro técnico; retoma o que faltou.
...
```
STATUS:      BLOCKED
TASK_ID:     T-20260907-003
OWNER:       marketing-nar
SUMMARY:     O inventario dos assets da campanha de matricula parou na metade. A tool de acervo passou a retornar erro ao abrir a pasta institucional e o erro se repetiu em nova tentativa. Nenhuma alternativa foi tentada.
OUTPUT:      { assets_inventariados: 4, assets_pendentes: 9 }
BLOCKERS:    - eduinfo_get_asset retornou MCP_TOOL_ERROR ao abrir o Folder_ID institucional autorizado; o erro se repete e impede concluir o inventario dos 9 assets restantes. Desbloqueio depende de resolver a falha da tool, nao de nova tentativa.
NEXT_OWNER:  rufas-router
```
