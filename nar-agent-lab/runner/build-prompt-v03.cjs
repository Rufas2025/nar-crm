#!/usr/bin/env node
/**
 * Construtor de prompts do NAR-ROUTER-BENCHMARK-V0.3.
 *
 * BLINDING ESTRUTURAL, mesmo desenho do v0.2: este arquivo lê APENAS
 * `router-cases-v0.3.intents.json`. O gabarito vive em
 * `router-cases-v0.3.expected.json`, que este script nunca abre.
 *
 * Uso: node build-prompt-v03.cjs          (gera prompts-v03/W-XX.md)
 *      node build-prompt-v03.cjs --check  (audita blinding + ancoragem + originalidade)
 */
const fs = require('fs');
...
const INTENTS = 'benchmarks/v0.3/router-cases-v0.3.intents.json';
const EXPECTED = 'benchmarks/v0.3/router-cases-v0.3.expected.json'; // só para --check

const intents = J(INTENTS);
const registry = J('capabilities/capability-registry.json');
const schema = R('runner/output-schema.json');
const OUT = path.join(__dirname, 'prompts-v03');
...
if (process.argv.includes('--check')) {
  ...
  if (intents.cases.length !== 42) flag(`esperado 42 casos, achei ${intents.cases.length}`);
  ...
  // 2. ancoragem: nenhuma intenção v0.3 pode estar no spec do router
  for (const c of intents.cases) {
  ...
  // 3. originalidade contra v0.1 E contra as intenções substituídas do v0.2
  ...
  const REPLACED_V02_IDS = new Set(['V-09', 'V-10', 'V-16', 'V-19', 'V-32', 'V-34']);
  const otherIntents = [
    ...v01.cases.map(c => c.intent),
    ...v02.cases.filter(c => REPLACED_V02_IDS.has(c.id)).map(c => c.intent),
  ];
  ...
  for (const c of newOrCorrected) {
    ...
    for (const o of otherIntents) {
      ...
      if (jac > 0.4) flag(`${c.id} parafraseia um caso substituído/v0.1 (Jaccard ${jac.toFixed(2)})`);
    }
  }
  ...
  const preserved = exp.cases.filter(c => c.source.startsWith('preserved:')).length;
  const corrected = exp.cases.filter(c => c.source.startsWith('corrected:')).length;
  const brandNew = exp.cases.filter(c => c.source === 'new').length;
  console.log(bad
    ? `\n${bad} FALHAS`
    : `\nV0.3 OK — ${intents.cases.length} casos (${preserved} preservados, ${corrected} corrigidos, ${brandNew} novos)\n` +
      `  blinding: gabarito em arquivo separado, nenhum fragmento nos prompts\n` +
      `  ancoragem: 0 intenções presentes no rufas-router.md\n` +
      `  originalidade: 0 paráfrases contra v0.1 e contra os 6 casos v0.2 substituídos\n` +
...
console.log(`${intents.cases.length} prompts v0.3 gerados em runner/prompts-v03/`);
