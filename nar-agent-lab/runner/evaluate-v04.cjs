#!/usr/bin/env node
/**
 * Avalia os outputs do RUN_01 do benchmark v0.4 contra o gabarito v0.4.
 *
 * Lê EXCLUSIVAMENTE:
 *   - benchmarks/v0.4/router-cases-v0.4.intents.json (só para contagem/sanidade)
 *   - benchmarks/v0.4/router-cases-v0.4.expected.json (gabarito)
 *   - capabilities/capability-registry.json (tool policy)
 *   - runner/outputs/v04-run-01/*.json (outputs a avaliar)
 *
 * NÃO lê nem escreve nada do v0.1, v0.2 ou v0.3 — nem router-cases.json, nem
 * results (v0.1/v0.2/v0.3), nem os diretórios de outputs de nenhum desses RUNs, nem
 * evaluate.cjs / evaluate-v02.cjs / evaluate-v03.cjs. Isso é verificado em runtime abaixo.
 *
 * Não modifica nenhum output cru — apenas lê e agrega em runner/results-v04/.
 *
 * Uso: node evaluate-v04.cjs [--run 01] [--dir <pasta de outputs>]
 */
const fs = require('fs');
...
const FORBIDDEN_PATHS = [
  ...
  // v0.3
  'benchmarks/v0.3/', 'runner/evaluate-v03.cjs', 'runner/results-v03/',
  'runner/outputs/v03-run-01',
];
// Guarda dura: este script não pode ler nenhum artefato do v0.1, v0.2 ou v0.3.
...
const outDir = dirArg > -1
  ...
  : path.join(__dirname, 'outputs', `v04-run-${RUN}`);

const expectedFile = J('benchmarks/v0.4/router-cases-v0.4.expected.json');
const intentsFile = J('benchmarks/v0.4/router-cases-v0.4.intents.json');
...
function classify(c, out) {
  ...
  // CROSS_OWNER_DEPENDENCY: nos casos da categoria "dependencia" cujo gabarito de fato
  // exige uma cadeia de 2+ owners, essa cadeia real (owner A → owner B com DEPENDENCIES
  // apontando para a task de A) precisa existir. Quando o gabarito da categoria é
  // escalação pura (expected.decomposition < 2 — ex.: capability-gap que zera as
  // tasks), não há cadeia a cobrar: a checagem fica de fora (null), do mesmo jeito que
  // já acontece para toda categoria que não é "dependencia". Corrige o bug do v0.3
  // (evaluate-v03.cjs) que forçava cross_owner_dep_ok=false nesse caso mesmo quando o
  // modelo acertava a escalação em tudo (decomp_ok=true, owner_ok=true).
  let cross_owner_dep_ok = null;
  if (c.category === 'dependencia' && c.expected.decomposition >= 2) {
...
if (cases.length !== 44) { console.error(`ESPERADO 44 casos no gabarito, achei ${cases.length}`); process.exit(1); }
...
if (missing.length) {
  console.error(`OUTPUTS AUSENTES OU INVÁLIDOS (${missing.length}/44): ${missing.join(', ')}`);
...
// Denominador só conta casos "dependencia" onde a checagem realmente se aplica
// (cross_owner_dep_ok !== null, i.e. expected.decomposition >= 2) — um caso
// "dependencia" que é escalação pura no gabarito não entra nem no numerador nem no
// denominador, mesma lógica de toda métrica baseada em campo que pode ser null.
const depCases = results.filter(r => r.category === 'dependencia' && r.cross_owner_dep_ok !== null);
...
const report = {
  run: `v04-${RUN}`, suite: 'router-cases-v0.4', model: 'sonnet',
...
fs.mkdirSync(path.join(__dirname, 'results-v04'), { recursive: true });
fs.writeFileSync(path.join(__dirname, 'results-v04', `run-${RUN}.json`), JSON.stringify(report, null, 2) + '\n');

console.log(`V0.4 RUN_${RUN} — ${N} casos | PASS ${PASS} · PARTIAL ${PARTIAL} · FAIL ${FAIL}`);
